...
%%load transcriptomic profiles
load('DatasetsnewFC.mat')
%% choose flux distribution to compare
load('flux_matrixP1.mat')
%load('flux_matrixP2.mat')
%load('fluxATP.mat')

all_objpairs = [flux_matrixP1; DatasetsnewFC];
all_solutions = [flux_matrixP1; DatasetsnewFC];
%%
...
if strcmp(answer,'y')
    ...
    for NoClust = 2:30
        [cidx, ctrs] = kmeans(genes_vs_profiles, NoClust, 'dist','corr', 'rep',5, 'disp','final'); %kmeans_corr_1 is my modified version of pdist, in which we use x - 1 instead of x - mean(x) all over the place in the definition of the correlation distance here  http://www.mathworks.co.uk/help/stats/pdist.html?refresh=true
        %figure
...
[cidx, ctrs] = kmeans(genes_vs_profiles, num_clusters, 'dist','corr', 'rep',5, 'disp','final'); %kmeans_corr_1 is my modified version of pdist, in which we use x - 1 instead of x - mean(x) all over the place in the definition of the correlation distance here  http://www.mathworks.co.uk/help/stats/pdist.html?refresh=true
figure
